  ...
  if (name === 'blocked') {
    return {
      ...
      conflicts: {
        version: 5,
        status: 'blocked',
        conflicts: [warningConflict, criticalConflict],
        hasCritical: true,
        canUnblock: false,
        unavailableChecks: [],
      },
...
const masterUser = {
  ...
  type: 'professor',
  profile: { id: 'profile-professor-1', name: 'Prof. Bruno' },
  professor: {
    id: 'professor-1',
    contractId: 'contract-1',
    role: 'master',
    collaboratorFunction: { id: 'function-1', code: 'master', name: 'Master' },
  },
  accessControl: { isMaster: true, permissions: [] },
};

function historyFixture(current) {
  const approvedVersion = versionFixture('approved', 2, []);
  const draftVersion = versionFixture('draft', 1, []);
  return {
    assemblyId: current.id,
    currentVersion: current.currentVersion,
    versions: [draftVersion, approvedVersion, current.latestVersion],
    auditTrail: [],
  };
}
...
async function installSessionAndApiMocks(page, scenario, options = {}) {
  const fixture = scenarioFixture(scenario);
  await page.addInitScript(({ user }) => {
    localStorage.setItem('token', 'issue-318-browser-evidence-token');
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (method === 'GET' && pathname.endsWith('/capacity-prescriptions/alunos/aluno-1')) {
      return fulfill(capacities);
    }
    if (method === 'GET' && pathname.endsWith('/consolidated-prescriptions/alunos/aluno-1/conflicts')) {
      return fulfill(fixture.conflicts);
    }
    if (method === 'GET' && pathname.endsWith('/consolidated-prescriptions/alunos/aluno-1/history')) {
      return fulfill(historyFixture(fixture.assembly));
    }
    if (method === 'GET' && pathname.endsWith('/consolidated-prescriptions/alunos/aluno-1')) {
      return fulfill(fixture.assembly);
    }
    if (
      options.forceConflict409 &&
      method === 'PATCH' &&
      pathname.endsWith('/consolidated-prescriptions/alunos/aluno-1/composition')
    ) {
...
async function assertNoHorizontalOverflow(page, label) {
  ...
  assert.ok(
    ...
    `${label}: overflow horizontal (${metrics.scrollWidth}px > ${metrics.viewportWidth}px)`
  );
}
...
async function scanA11y(page, label) {
  ...
  fs.writeFileSync(
    path.join(OUTPUT_DIR, `${label}-axe.json`),
    JSON.stringify({ violations: result.violations, passes: result.passes.map((item) => item.id) }, null, 2)
  );
  assert.equal(
    result.violations.length,
    0,
    `${label}: axe encontrou ${result.violations.length} violação(ões): ${result.violations.map((item) => item.id).join(', ')}`
  );
}
...
async function verifyScenario(browser, scenario, deviceLabel, viewport) {
  ...
  await installSessionAndApiMocks(page, scenario);
  await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: 'networkidle' });
  await page.getByRole('heading', { name: 'Montagem Consolidada da Prescrição' }).waitFor();
  ...
  await assertNoHorizontalOverflow(page, `${deviceLabel}-${scenario}`);
  ...
  if (scenario === 'blocked') {
    ...
    assert.ok(styles, `${deviceLabel}-${scenario}: não foi possível medir estilos de warning/critical`);
    assert.notEqual(styles.warningBackground, styles.criticalBackground, 'warning e critical precisam de fundos distintos');
    assert.notEqual(styles.warningBorder, styles.criticalBorder, 'warning e critical precisam de bordas distintas');
    ...
    if (scenario === 'ready_for_review') {
      await page.getByRole('button', { name: 'Aprovar montagem' }).waitFor();
    }
  }
  ...
  assert.deepEqual(browserErrors, [], `${label}: erros JavaScript no navegador: ${browserErrors.join(' | ')}`);
  await context.close();
}
...
async function verifyConcurrency409(browser) {
  ...
  const page = await context.newPage();
  await installSessionAndApiMocks(page, 'draft', { forceConflict409: true });
  await page.goto(`${BASE_URL}${ROUTE}`, { waitUntil: 'networkidle' });
  await page.getByRole('heading', { name: 'Montagem Consolidada da Prescrição' }).waitFor();
