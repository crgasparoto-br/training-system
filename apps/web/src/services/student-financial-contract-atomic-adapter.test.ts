...
const contractLink = (alunoId = 'student-1'): StudentContractLink => ({
  id: 'link-1',
  alunoId,
...
const buildService = () => ({
  ...
  listStudentContracts: vi.fn(async (alunoId: string): Promise<AlunoContractsResponse> => ({
    alunoId,
    activeContract: null,
    contracts: [],
  })),
  linkStudentContract: vi.fn(async (alunoId: string) => contractLink(alunoId)),
  updateStudentContract: vi.fn(async (alunoId: string) => contractLink(alunoId)),
  activateStudentContract: vi.fn(async (alunoId: string) => contractLink(alunoId)),
});
...
describe('student financial contract atomic adapter', () => {
  ...
  it('does not persist the profile separately when the atomic update fails', async () => {
    ...
    const uninstall = installStudentFinancialContractAtomicAdapter(service, atomicService, root);
  ...
  it('uses the selected contract for an existing link update and consumes legacy follow-up calls', async () => {
    const service = buildService();
    const atomicUpdate = vi.fn(async () => ({
    ...
    expect(atomicUpdate).toHaveBeenCalledWith('student-1', { age: 32 }, {
      contractId: 'contract-1',
      endDate: '2027-07-01',
    });
