...
export function ProtocolNavTabs({ activeSlug, alunoId }: { activeSlug: ProtocolSlug | string; alunoId: string }) {
  return (
    <nav aria-label="Protocolos de avaliação física" className="flex flex-wrap gap-2">
      {protocolNavItems.map(([slug, label]) => (
        <Link
          ...
          className={
            slug === activeSlug
              ? 'rounded-lg border border-primary bg-primary/10 px-3 py-2 text-sm font-medium text-primary'
