...
vi.mock('../../services/adipometry.service', () => ({
  adipometryService: {
    listAccessibleStudents: mocks.listAccessibleStudents,
...
function assessment(id: string, alunoId: string, assessmentDate = '2026-08-04'): AdipometryAssessmentDetail {
  return {
    id,
...
describe('useAdipometryWorkspace context isolation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.listAccessibleStudents.mockResolvedValue([]);
  ...
  it('carrega alunos pela fronteira ADPT autorizada', async () => {
    const student = { id: 'aluno-a', user: { profile: { name: 'Aluno A' } } };
    mocks.listAccessibleStudents.mockResolvedValue([student]);
    const { result } = renderHook(() => useAdipometryWorkspace({
      canView: true,
      lockedAlunoId: '',
      assessmentIdParam: '',
    }));
    await waitFor(() => expect(result.current.alunos).toEqual([student]));
    expect(mocks.listAccessibleStudents).toHaveBeenCalledTimes(1);
  });
