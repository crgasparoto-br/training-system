...
export function ProntuarioScreen() {
  ...
  const hasRecords = (overview?.records.length || 0) > 0;
  const hasParqSubmissions = parqSubmissions.length > 0;
  const hasPositiveItems = latestPositiveItems.length > 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedAlunoId && !loading ? (
        <div className="space-y-6">
          {!hasRecords ? (
            <div className="rounded-md border border-primary/20 bg-primary/5 px-4 py-4 text-sm text-foreground">
              <div className="font-medium text-primary">Primeiro registro PRNT deste aluno</div>
              <p className="mt-1 text-muted-foreground">
                Comece pelo resumo e salve o cabeçalho para liberar o histórico profissional, acompanhamentos e snapshots deste aluno.
              </p>
            </div>
          ) : null}
              ...
              {blocks.summary && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{currentRecord ? `${currentRecord.code} · ${currentRecord.status}` : 'Novo registro PRNT para iniciar o histórico profissional do aluno.'}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {currentRecord ? (
                      ...
                      <div className="rounded-md border border-dashed border-border px-3 py-2 text-xs text-muted-foreground">
                        Ainda não existe registro salvo para este aluno. Preencha o resumo inicial e salve o cabeçalho para começar.
                      </div>
                    )}
              ...
              {blocks.anamnesis && (
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    {hasPositiveItems ? latestPositiveItems.map((item) => (
                      ...
                      <div className="rounded-md border border-dashed border-border px-4 py-3 text-sm text-muted-foreground">
                        {hasParqSubmissions
                          ? 'A submissão PAR-Q selecionada não trouxe itens positivos. O PRNT continua disponível para registrar resumo, rotina, dores e demais acompanhamentos profissionais.'
                          : 'Este aluno ainda não tem submissões PAR-Q no histórico. O PRNT pode ser iniciado normalmente, e os acompanhamentos da anamnese aparecerão aqui assim que houver respostas registradas.'}
                      </div>
                    )}
                    {hasPositiveItems ? <Button type="button" variant="outline" onClick={saveAnamnesisFollowUps} disabled={saving}><ClipboardList size={16} />Criar/atualizar acompanhamentos</Button> : null}
                  </CardContent>
                </Card>
              )}
                ...
                <CardContent className="space-y-2">
                  ...
                  {!hasRecords ? (
                    <div className="rounded-md border border-dashed border-border px-3 py-3 text-sm text-muted-foreground">
                      Nenhum registro PRNT salvo ainda. Assim que o cabeçalho for criado, o histórico deste aluno aparecerá aqui.
                    </div>
                  ) : null}
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  ...
                  {selectedParqSubmission ? (
                    ...
                    <div className="rounded-md border border-dashed border-border px-3 py-3 text-sm text-muted-foreground">
                      Sem submissões históricas de PAR-Q para este aluno até o momento.
                    </div>
                  )}
                </CardContent>
              </Card>
