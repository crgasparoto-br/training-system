...
const CONTRACT_START_DATE_FIELD = 'intakeForm.financialInfo.contractStartDate';
...
const getContractStartDateControl = () =>
  document.querySelector<HTMLInputElement>(`[name="${CONTRACT_START_DATE_FIELD}"]`);

const findReplacementButton = () => {
  const panel = document.getElementById(CONTRACT_REPLACEMENT_PANEL_ID);
  return panel?.querySelector<HTMLButtonElement>('button') || null;
};
...
export function AlunoFormWithContractLifecycle() {
  ...
  const [currentContractPreviewTitle, setCurrentContractPreviewTitle] = useState('Contrato vigente');
  const [currentContractPreviewError, setCurrentContractPreviewError] = useState<string | null>(null);

  const activeDocumentSignedAt = activeContract?.contract.signedAt || activeContract?.signedAt || null;
  const currentContractIsSigned = Boolean(activeDocumentSignedAt || activeContract?.contract.status === 'SIGNED');
  const isDifferentSelection = Boolean(
    activeContract && selectedContractId && activeContract.contractId !== selectedContractId
  );

  const replacementSummary = useMemo(() => {
    if (!isDifferentSelection) return null;

    const plannedStart = plannedStartDate ? formatDateLabel(`${plannedStartDate}T00:00:00`) : null;
    return plannedStart
      ? `O novo contrato está planejado para iniciar em ${plannedStart}. A entrada em vigor nunca ocorrerá antes da assinatura.`
  ...
  useEffect(() => {
    const syncControls = () => {
      const contractSelect = getSelectedContractControl();
      const startDateControl = getContractStartDateControl();
      const selectedValue = contractSelect?.value?.trim() || '';

      setSelectedContractId(selectedValue);
      setSelectedContractLabel(
        selectedValue ? contractSelect?.selectedOptions[0]?.textContent?.trim() || '' : ''
      );
      setPlannedStartDate(startDateControl?.value || '');
    };

    syncControls();
    document.addEventListener('change', syncControls);
    document.addEventListener('input', syncControls);
    const observer = new MutationObserver(syncControls);
    observer.observe(document.body, { childList: true, subtree: true });

    return () => {
      document.removeEventListener('change', syncControls);
      document.removeEventListener('input', syncControls);
      observer.disconnect();
    };
  }, []);
  ...
  useEffect(() => {
    const syncLifecycleSlot = () => {
      ...
      const selectorBlock = directChildren.find((child) =>
        child.querySelector(`select[name="${SELECTED_CONTRACT_FIELD}"]`)
      ) || directChildren[1];
  ...
  useEffect(() => {
    ...
    const syncLegacyPresentation = () => {
      const contractSection = document.getElementById(CONTRACT_SECTION_SLOT_ID);
      if (!contractSection) return;

      Array.from(contractSection.children).forEach((child) => {
        if (!(child instanceof HTMLElement) || child.id === CONTRACT_LIFECYCLE_SLOT_ID) return;
        const text = child.textContent?.replace(/\s+/g, ' ').trim() || '';
        if (
          text.startsWith('Contrato ativo atual:') ||
          text === 'Este aluno já possui um contrato ativo. Ao ativar um novo contrato, o anterior será encerrado.'
        ) {
      ...
      const replacementPanel = document.getElementById(CONTRACT_REPLACEMENT_PANEL_ID);
      if (replacementPanel) {
        replacementPanel.classList.add('hidden');
        hiddenElements.add(replacementPanel);
      }

      const replacementButton = findReplacementButton();
      setReplacementRequired(Boolean(replacementPanel) || isDifferentSelection);
      setReplacementConfirmed(
        Boolean(replacementButton?.textContent?.includes('Troca confirmada'))
      );
    };

    syncLegacyPresentation();
    const observer = new MutationObserver(syncLegacyPresentation);
  ...
  }, [isDifferentSelection, selectedContractId]);
  ...
  const handleConsultCurrentContract = async () => {
    ...
    try {
      ...
      setCurrentContractPreviewTitle(document.title || activeContract.contract.title || 'Contrato vigente');
      setCurrentContractPreview(document.renderedHtml);
    } catch (error: any) {
      setCurrentContractPreviewError(
        error?.response?.data?.error || 'Não foi possível abrir o contrato vigente.'
      );
    } finally {
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50/40 p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Este é o vínculo que continua valendo até a entrada em vigor de outro contrato assinado.
                  </p>
                </div>
                {activeContract && (
                  <Button
              ...
              {activeContractLoading ? (
                <p className="mt-4 text-sm text-muted-foreground">Consultando vínculo vigente...</p>
              ) : activeContractError ? (
                ...
                <div className="mt-4 space-y-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-base font-semibold text-foreground">{activeContract.contract.title}</p>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                    ...
                    <div className="rounded-lg border border-emerald-200 bg-white/80 px-4 py-3">
                      <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Início da vigência</p>
                      <p className="mt-1 text-sm font-semibold text-foreground">
                        {formatDateLabel(activeContract.startDate)}
                      </p>
                    </div>
                    <div className="rounded-lg border border-emerald-200 bg-white/80 px-4 py-3">
                      <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Término</p>
                      <p className="mt-1 text-sm font-semibold text-foreground">
                        {activeContract.endDate ? formatDateLabel(activeContract.endDate) : 'Em vigor'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <p className="mt-4 rounded-lg border border-border bg-white/70 px-4 py-3 text-sm text-muted-foreground">
                  O aluno ainda não possui um contrato vigente. Um novo vínculo só será ativado após assinatura e início da vigência.
                </p>
              )}
            ...
            {replacementRequired && activeContract && isDifferentSelection && (
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <h4 className="flex items-center gap-2 text-sm font-semibold text-foreground">
                      ...
                      Novo contrato em substituição
                    </h4>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {selectedContractLabel || 'Novo contrato selecionado'}
                    </p>
                  </div>
                  <Button
                    type="button"
                    variant={replacementConfirmed ? 'outline' : 'default'}
                  ...
                  >
                    ...
                    {replacementConfirmed ? 'Substituição confirmada' : 'Confirmar preparação da substituição'}
                  </Button>
                </div>

                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div className="rounded-lg border border-current/10 bg-white/70 px-4 py-3">
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Início planejado</p>
                    <p className="mt-1 text-sm font-semibold text-foreground">
                      {plannedStartDate ? formatDateLabel(`${plannedStartDate}T00:00:00`) : 'Na assinatura'}
                    </p>
                  </div>
                  <div className="rounded-lg border border-current/10 bg-white/70 px-4 py-3">
                    <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Contrato vigente até</p>
                    <p className="mt-1 text-sm font-semibold text-foreground">
                      {plannedStartDate
                        ? `Entrada efetiva do novo contrato, não antes de ${formatDateLabel(`${plannedStartDate}T00:00:00`)}`
                        : 'Assinatura e ativação do novo contrato'}
                    </p>
                  </div>
                </div>

                <p className="mt-3 text-sm text-muted-foreground">{replacementSummary}</p>
                <p className="mt-2 text-xs font-medium text-foreground">
                  Se o novo contrato for recusado, expirar ou for cancelado, o contrato vigente acima permanece inalterado.
                </p>
              </div>
            )}
          </div>,
      ...
      {currentContractPreview &&
        createPortal(
          <div
            ...
            role="presentation"
            onMouseDown={(event) => {
              if (event.target === event.currentTarget) setCurrentContractPreview('');
            }}
          >
            <div
              role="dialog"
            ...
            >
              <div className="flex items-center justify-between gap-4 border-b border-border px-5 py-4">
                <div>
                  <h2 id="current-contract-preview-title" className="text-lg font-semibold text-foreground">
                    {currentContractPreviewTitle}
                  </h2>
                  <p className="text-sm text-muted-foreground">Contrato atualmente vigente, em modo somente leitura.</p>
                </div>
                <Button
                  type="button"
                  variant="ghost"
