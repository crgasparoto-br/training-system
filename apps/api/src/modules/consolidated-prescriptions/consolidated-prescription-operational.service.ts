...
import {
  buildBalanceWorkoutDayOperationalBlock,
  buildFlexibilityWorkoutDayOperationalBlock,
...
export function buildOperationalProjectionItems(
  ...
  for (const version of capacityVersions) {
    ...
    if (parameters.type === 'flexibility') {
      ...
      const operationalBlock = buildFlexibilityWorkoutDayOperationalBlock(version.id, flexibility);
      const detailNotes = operationalBlock ? formatFlexibilityWorkoutDayNote(flexibility) : null;
      items.push({
        key: `${version.id}:flexibility`,
        capacity: 'flexibility',
        capacityPrescriptionVersionId: version.id,
        target: operationalBlock ? 'WorkoutDay' : 'none',
        compatibility: operationalBlock ? 'mapped' : 'incompatible',
        incompatibilityCode: operationalBlock ? null : 'operational_representation_unavailable',
        incompatibilityMessage: operationalBlock
          ? null
        ...
        proposedFields: operationalBlock
          ? {
              WorkoutDay: { detailNotes },
              WorkoutDayCapacityOperationalBlock: operationalBlock,
            }
          : {},
        unsupportedParameters: operationalBlock ? [] : Object.keys(flexibility),
    ...
    const operationalBlock = buildBalanceWorkoutDayOperationalBlock(version.id, balance);
    const complementNotes = operationalBlock ? formatBalanceWorkoutDayNote(balance) : null;
    items.push({
      ...
      proposedFields: operationalBlock
        ? {
            WorkoutDay: { complementNotes },
            WorkoutDayCapacityOperationalBlock: operationalBlock,
          }
        : {},
      unsupportedParameters: operationalBlock ? [] : Object.keys(balance),
...
export function createConsolidatedPrescriptionOperationalService(client: PrismaClient = prisma) {
  async function getProjection(
    ...
    const substituteIds = Array.from(
      new Set(Array.from(substitutions.values()).map((item) => item.substituteExerciseLibraryId))
    );
  ...
  return {
    ...
    async setExerciseMapping(
      ...
      try {
        ...
        if (error instanceof Error && error.name === 'CapacityPrescriptionDomainError') {
    ...
    async createExerciseSubstitution(
      ...
      const criticalResistedConflicts = assembly.latestVersion.conflicts.filter(
        (item) => item.severity === 'critical' && item.affectedCapacities.includes('resisted')
      );
      if (criticalResistedConflicts.length) {
        invalid('Resolva os impedimentos críticos da capacidade resistida antes de registrar substituição');
      }
      ...
      const updated = await consolidatedPrescriptionService.updateComposition(
        context,
        {
          ...
          dataRefs: [...cloneAdditionalRefs(assembly.latestVersion.dataRefs), substitutionRef],
          technicalObservation: assembly.latestVersion.technicalObservation ?? null,
          professorJustification: assembly.latestVersion.professorJustification,
          studentInstruction: assembly.latestVersion.studentInstruction ?? null,
        },
