...
import {
  ...
  type AdipometryProfileAuthority,
} from './adipometry-clinical-integrity.js';
import {
  adipometryMeasurementsFromRow,
  getAdipometryAnthropometrySupport,
  getAdipometryApprovedProtocol,
  getAdipometryProfile,
  getPersistedAdipometryDecision,
  type AdipometryAssessmentRow,
  type AdipometryApprovedProtocolRow,
  type AdipometryDbClient,
...
function addDecisionIncompatibility(
  compatibility: AdipometryPreviewResult['compatibility'],
  reason: AdipometryPreviewResult['compatibility']['reasons'][number] | null
) {
  if (
    !reason
    || compatibility.reasons.some(
      (item) => item.code === reason.code && item.field === reason.field
    )
  ) {
  ...
  return {
    ...compatibility,
    compatible: false,
    reasons: [...compatibility.reasons, reason],
  };
}

export async function buildHardenedAdipometryPreview(
  client: AdipometryDbClient,
  contractId: string,
  actorUserId: string,
  row: AdipometryAssessmentRow,
  options: { lockAuthorities?: boolean } = {}
  ...
  if (options.lockAuthorities) {
    protocol = await getAdipometryApprovedProtocol(
      client,
      contractId,
      row.protocolCode,
      row.protocolVersion,
    ...
    anthropometrySupport = await getAdipometryAnthropometrySupport(
      client,
      contractId,
      row.alunoId,
      assessmentDate,
  ...
  const decisionReason = getProtocolSexSourceIncompatibility({
    profile,
  ...
  const compatibility = addDecisionIncompatibility(
    calculated.compatibility,
    decisionReason
  );
  const calculationSnapshot =
    calculated.calculationSnapshot && compatibility.compatible
      ? applyPersistedProtocolSexDecision(
          calculated.calculationSnapshot,
          decision
        )
      : undefined;
  ...
  const usedSkinfolds: AdipometrySkinfoldField[] = calculationContext.protocolSex
    ? [...(definition.calculationSkinfoldsBySex?.[sexKey] ?? [])]
    : [];
  const legacyFingerprint = buildAdipometryInputFingerprint({
    assessmentId: row.id,
    assessmentDate,
  ...
  const inputFingerprint = buildAdipometryClinicalFingerprint({
    legacyFingerprint,
    profile,
    ageAtAssessment: calculationSnapshot?.ageAtAssessment ?? null,
    decision,
  });
  ...
  return {
    protocol: {
      ...
      status: 'APPROVED',
      compatibility,
      ...buildAdipometryProtocolPresentation(definition, compatibility),
    },
    normalizedMeasurements: calculationContext.measurements,
    usedSkinfolds,
    compatibility,
    ...(compatibility.compatible && calculated.results
...
export function assertAdipometryProtocolSexSource(
  profile: AdipometryProfileAuthority,
  protocolSex: AdipometryProtocolSex,
  source: AdipometryProtocolSexSource | undefined,
  overrideReason: string | null | undefined
) {
  const reason = getProtocolSexSourceIncompatibility({
    profile,
    protocolSex,
    source: source ?? 'professional_confirmation',
    overrideReason: overrideReason ?? null,
  });
  if (reason) {
    throw new AdipometryServiceError(reason.message, reason.code, 400);
  }
}
